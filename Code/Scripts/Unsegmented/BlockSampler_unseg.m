...
function [Aest,mu,cov,Pi_Gmm,state_three_prob] = BlockSampler_unseg(X,seqlen,Aest,mu,cov,beta_Aest,Pi,Pi_Gmm,globmu,globcov)
...
% return: updated attributes, state_three_probs: the values of 'z' in the cascaded transition matrix
    ...
    for n=1:Num_sequences
        for k = 1:Nstates
            SamplePerState(n,k)=sum(allsamplestate{n,:} == k);
        end
    end
...
% Count total number of state-3 samples to get z
        state_three = sum(SamplePerState);
        state_three_prob = (state_three(end)-Num_sequences)/state_three(end);
