...
/*
   ...
   * Usage: Probs = HMMlikelihoodCalC_UK(i-1,NHmm+1,Pi,X,Aest,mu,cov,Pi_Gmm);
   * where:
...
double* State_Prob(double *B,double *tmp_X,double *tmp_mu,double *tmp_cov,double *tmp_PiGmm,
    ...
    for(unsigned int i=0;i<nstates;++i){
        for(unsigned int j=0;j<nsamples;++j){
            double *sums = (double *)mxMalloc(sizeof(double)*Nmix);
...
double Calc_Forward(double *B,double *tmp_Aest,double *Pi,int NumObs,int NumStates){
    ...
    double *alpha = (double *)mxMalloc(sizeof(double)*NumStates*NumObs);
...
{                
    ...
    cell_element_ptr_Aest = mxGetCell(prhs[4],0);     
    const int *dims_Aest = mxGetDimensions(cell_element_ptr_Aest); // Get Dimensions of Aest
    
    cell_element_ptr_PiGmm = mxGetCell(prhs[7],0);     
    ...
    for(unsigned int i=0;i<nhmm;++i){       // Goes over all HMMs
       ...
       double *B = (double *)mxMalloc(sizeof(double)*dims_Aest[0]*dims_X[1]);
